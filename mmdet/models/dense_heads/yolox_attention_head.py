...
class YOLOXAttentionHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        self.multihead_attn = nn.MultiheadAttention(embed_dim= in_channels, num_heads = 1)
    ...
    def _build_attention(self, x):
        N, C, H, W = x.shape
        # convert (N, C, H, W) => (N, C, H*W) => (H*W, N, C)
        x = torch.flatten(x, start_dim= 2).permute(2, 0, 1)
        attn_output, attn_output_weights = self.multihead_attn(x, x, x)

        # return back (H*W, N, C) => (N, C, H*W) => (N, C, H, W)
        attn_output = attn_output.permute(1, 2, 0)
        # attn_output = nn.unflatten(attn_output, dim = 2, sizes = (H, W))
        attn_output = attn_output.view(N, C, H, W)

        return attn_output

    def init_weights(self):
        super(YOLOXAttentionHead, self).init_weights()
    ...
    def forward_single(self, x, cls_convs, reg_convs, conv_cls, conv_reg,
        ...
        attn_output = self._build_attention(x)
    
        cls_feat = cls_convs(attn_output)
        reg_feat = reg_convs(attn_output)
